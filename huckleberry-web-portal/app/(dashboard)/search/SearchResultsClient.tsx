...
export function SearchResultsClient({
  ...
  useEffect(() => {
    async function performSearch() {
      if (!initialQuery && !initialDateFrom && !initialDateTo && initialTags.length === 0) {
      ...
      try {
        ...
        if (initialDateFrom) params.append("dateFrom", initialDateFrom);
        if (initialDateTo) params.append("dateTo", initialDateTo);
        if (initialTags.length > 0) params.append("tags", initialTags.join(","));
    ...
    performSearch();
  }, [initialQuery, initialDateFrom, initialDateTo, initialTags.join(",")]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        {(initialQuery || initialDateFrom || initialDateTo || initialTags.length > 0) && (
          <div className="text-sm text-gray-600 dark:text-neutral-400">
            {initialQuery && <span>Query: &quot;{initialQuery}&quot;</span>}
            {initialDateFrom && <span>{initialQuery ? " • " : ""}From: {initialDateFrom}</span>}
            {initialDateTo && <span> • To: {initialDateTo}</span>}
            {initialTags.length > 0 && <span> • Tags: {initialTags.join(", ")}</span>}
          </div>
        )}
